...
@pytest.mark.skip(reason="PR #33: enforce_crs not present in livrable_topology module")
...
@pytest.mark.skip(reason="PR #33: PR32 C0 test skipped")
...
@pytest.mark.skip(reason="PR #33: C0 connectors disabled")
...
@pytest.mark.skip(reason="PR #33: C0 connectors no longer created")
...
def test_t_junction_source_after_target():
    """When the source line (A) comes AFTER the target line (B) in the list,
    and As endpoint projects onto B, the source endpoint must still be
    rewritten to the exact split point on B."""
    df = _df([
        _livrable_row(geometry=LineString([(5, 0), (5, 10)]),
                       src='ft', mode_pose='7', infra_type='ft', pb_id='PB2'),
        _livrable_row(geometry=LineString([(0, 5), (5, 5)]),
    ...
    out, n_splits = lt._split_livrableedges_at_endpoint_projections(df, tol_m=0.5)
    assert len(out) >= 3, f'Expected >=3 rows after split, got {len(out)}'
    geoms = list(out.geometry)
    all_eps = []
    for g in geoms:
        cs = list(g.coords)
        all_eps.append((round(cs[0][0], 3), round(cs[0][1], 3)))
        all_eps.append((round(cs[-1][0], 3), round(cs[-1][1], 3)))
    count_55 = sum(1 for ep in all_eps if ep == (5.0, 5.0))
    assert count_55 >= 2, f'(5,5) should be shared endpoint >=2x, got {count_55}'


@pytest.mark.skip(reason="PR #33: C0 connectors disabled")
def test_energy_reconnect_existing_infra_stats():
    """When existing infra already reconnects after energy removal:
    0 C0 added, 0 energy_reconnectors_added, 1 energy_reconnected_by_existing."""
    df_before = _df([
        _livrable_row(geometry=LineString([(0, 0), (5, 0)]),
                       src='ft', mode_pose='7', infra_type='ft'),
        _livrable_row(geometry=LineString([(5, 0), (10, 0)]),
