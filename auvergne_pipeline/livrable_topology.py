...
def enforce_crs(gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """Force the CRS to EPSG:2154.

    If the gdf has no CRS, assume WGS84 (4326) and transform.
    If already 2154, return unchanged.
    """
    if gdf is None or gdf.empty:
        return gdf
    gdf = gdf.copy()
    if gdf.crs is None:
        log.warning("GeoDataFrame sans CRS — assume EPSG:4326 (WGS84)")
        gdf.set_crs(epsg=4326, inplace=True)
    if gdf.crs.to_epsg() != 2154:
        log.info("Converting CRS from %s → EPSG:2154", gdf.crs.to_epsg())
...
def _drop_c0_when_existing_equivalent(
    ...
    an existing (non-gc_neuf) edge passes within EXISTING_COINCIDENCE_M
    (2.0 m) and has a compatible angle (< 45 deg / > 135 deg).
    ...
    if non_c0_mask.sum() == 0:
        stats["c0_examined"] = int(c0_mask.sum())
        stats["c0_kept"] = stats["c0_examined"]
        if "length_m" in df.columns:
            stats["ign_route_delivered_as_gc_m_sum"] = float(
                df.loc[c0_mask, "length_m"].sum()
            )
        return df, stats
    ...
    for idx in df.index[c0_mask]:
        ...
        for _, erow in existing.iterrows():
            ...
            if egeom is None or egeom.is_empty or not isinstance(egeom, LineString):
                continue
            if geom.distance(egeom) >= EXISTING_COINCIDENCE_M:
                continue
    ...
    log.info(
        "=== QA — drop_c0 ===  examined=%d dropped=%d kept=%d gc_m_sum=%.1f",
        stats["c0_examined"], stats["c0_dropped"],
        stats["c0_kept"], stats["ign_route_delivered_as_gc_m_sum"],
    )
